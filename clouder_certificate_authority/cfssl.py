...
class CFSSL(object):
    ...
    def __init__(self, host, port, ssl=True):
        ...
        self.uri_base = '%s://%s:%s' % (ssl, host, port)
    ...
    def info(self, label, profile=None):
        ...
        Returns:
            (dict) Mapping with three keys:
                ...
                * usage: (list) a string array of key usages from the signing
                    profile.
    ...
    def new_key(self, hosts, names, common_name=None, key=None, ca=None):
        ...
        Returns:
            (dict) Mapping with three keys:
                * private key: (str) a PEM-encoded CA private key.
                * certificate: (str) a PEM-encoded self-signed CA certificate.
                * sums: (dict) Mapping holding both MD5 and SHA1 digests for the
                    certificate request
        """
        data = self._clean_mapping({
            'hosts': hosts,
    ...
    def _clean_mapping(self, mapping):
        ...
        return {k:v for k, v in mapping.iteritems()}
