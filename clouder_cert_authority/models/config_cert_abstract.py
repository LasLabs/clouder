...
class ClouderConfigCertAbstract(models.AbstractModel, API):
    """ It provides data handling for cert client + server configs. """

    _name = 'clouder.config.cert.abstract'
    _description = 'Clouder Config Cert Abstract'

    sign_default_id = fields.Many2one(
        string='Default Signing Policy',
        comodel_name='clouder.cert.policy.sign',
        required=True,
    )
    ...
    auth_policy_ids = fields.Many2many(
        ...
        comodel_name='clouder.cert.policy.auth',
