...
class ClouderCertRequest(models.Model, API):
    """ It provides the concept of a Cert Request """

    _name = 'clouder.cert.request'
    _inherit = ['clouder.cert.abstract',
                'clouder.key.abstract',
                ]
    _description = 'Clouder Cert Request'
    ...
    subject_info_ids = fields.Many2many(
        string='Names',
        comodel_name='clouder.cert.name',
        required=True,
    )
    ...
    def _new_cert(self, cert_data):
        ...
        for usage_str in cert_info['keyUsage']:
            ...
            if not usage:
                continue
            usages += usage
        cert = self.env['clouder.cert.x509'].create({
