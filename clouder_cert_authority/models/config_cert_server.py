...
class ClouderConfigCertServer(models.Model, API):
    """ It provides data handling for cert server configs. """

    _name = 'clouder.config.cert.server'
    _inherit = 'clouder.config.cert.abstract'
    _description = 'Clouder Config Cert Server'

    @api.multi
    def _compute_api_object(self):
        """ It computes the keys required for the JSON request. """
        for record in self:
            super(ClouderConfigCertServer, record)._compute_api_object()
